  ...
  return (
    <AbsoluteFill>
      ...
      <AbsoluteFill style={{ padding: "0 55px", display: "flex", flexDirection: "column" }}>
        ...
        <div style={{
          marginTop: 50, opacity: calendarOpacity, transform: `translateY(${calendarY}px)`,
          background: "rgba(255,255,255,0.04)", borderRadius: 24,
          border: "1px solid rgba(255,255,255,0.06)", padding: "28px 24px",
        }}>
          <p style={{ fontFamily, fontWeight: 800, fontSize: 28, color: "white", margin: "0 0 20px 0" }}>📅 Calendário de Festas</p>
          <div style={{ display: "flex", justifyContent: "space-around", marginBottom: 16 }}>
            {calendarDays.map((d, i) => (
              <span key={i} style={{ fontFamily, fontWeight: 700, fontSize: 20, color: "rgba(255,255,255,0.3)", width: 60, textAlign: "center" }}>{d}</span>
            ))}
          </div>
          {calendarDates.map((week, wi) => {
            ...
            return (
              <div key={wi} style={{ display: "flex", justifyContent: "space-around", marginBottom: 10, opacity: rowOpacity }}>
                {week.map((date, di) => {
                  ...
                  return (
                    <div key={di} style={{
                      ...
                      <span style={{ fontFamily, fontWeight: isEvent ? 800 : 400, fontSize: 22, color: isEvent ? "#f59e0b" : "rgba(255,255,255,0.4)" }}>{date}</span>
                    </div>
                  );
                })}
              </div>
        ...
        <div style={{
          marginTop: 22, opacity: eventsOpacity, transform: `translateY(${eventsY}px)`,
          background: "rgba(255,255,255,0.04)", borderRadius: 24,
          border: "1px solid rgba(255,255,255,0.06)", padding: "28px 24px",
        }}>
          <p style={{ fontFamily, fontWeight: 800, fontSize: 28, color: "white", margin: "0 0 20px 0" }}>🎉 Próximos Eventos</p>
          {[
            ...
            return (
              <div key={i} style={{
                ...
                padding: "14px 0", opacity: evtOpacity,
                borderBottom: i < 2 ? "1px solid rgba(255,255,255,0.04)" : "none",
              }}>
                <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
                  <span style={{ fontFamily, fontWeight: 700, fontSize: 20, color: "#f59e0b" }}>{evt.date}</span>
                  <span style={{ fontFamily, fontWeight: 600, fontSize: 22, color: "rgba(255,255,255,0.8)" }}>{evt.name}</span>
                </div>
                <span style={{
                  fontFamily, fontWeight: 600, fontSize: 16, color: "#f59e0b",
                  background: "rgba(245,158,11,0.12)", padding: "6px 14px", borderRadius: 8,
                }}>{evt.pkg}</span>
              </div>
            );
          })}
