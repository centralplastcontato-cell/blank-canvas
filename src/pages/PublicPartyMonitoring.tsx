...
export default function PublicPartyMonitoring() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setItems((data.items as unknown as MonitoringItem[]).map(i => ({ ...i })));
      setNotes(data.notes || "");
  ...
  const groupedItems = items.reduce<{ category: string; items: { item: MonitoringItem; idx: number }[] }[]>((acc, item, idx) => {
    const cat = item.category || "Geral";
    const last = acc[acc.length - 1];
    if (last && last.category === cat) {
      last.items.push({ item, idx });
    } else {
      acc.push({ category: cat, items: [{ item, idx }] });
    }
    return acc;
  }, []);
      ...
      <div className="min-h-screen flex items-center justify-center bg-background p-4">
        ...
        <Card className="max-w-md w-full">
          <CardContent className="py-12 text-center">
            ...
            <p className="text-muted-foreground text-sm">O acompanhamento da festa foi salvo com sucesso.</p>
          </CardContent>
        </Card>
      </div>
    );
      ...
      <div className="max-w-lg mx-auto px-4 py-6 space-y-5">
        ...
        <div className="text-center space-y-1">
          <div className="flex items-center justify-center gap-2">
            ...
            <h1 className="text-xl font-bold">Acompanhamento de Festa</h1>
          </div>
        ...
        {/* Monitoring items grouped by category */}
        {groupedItems.map(group => (
          <div key={group.category} className="space-y-3">
            <Label className="text-sm font-semibold block border-b pb-1">{group.category}</Label>
            {group.items.map(({ item, idx }) => (
              <Card key={idx} className={`transition-colors ${item.checked ? "border-primary/50" : ""}`}>
                <CardContent className="p-3 space-y-2">
                  <div className="flex items-start gap-3">
                    ...
                    />
                    <span className={`text-sm leading-snug ${item.checked ? "font-medium" : ""}`}>{item.label}</span>
                  </div>
                  {item.checked && (
                    <Input
        ...
        <div>
          ...
          <Textarea
            placeholder="Anotações adicionais sobre o acompanhamento..."
            value={notes}
