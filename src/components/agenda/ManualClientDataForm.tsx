...
export function ManualClientDataForm({ eventId, companyId, leadId, initialClientData, requestId, onSaved, onCancel }: ManualClientDataFormProps) {
  ...
  const lastSavedSnapshotRef = useRef(JSON.stringify(formData));
  ...
  const performSave = useCallback(async (data: ClientFormData) => {
    ...
    try {
      ...
      const childrenWithData = data.birthday_children.filter((child) => child.name);
  ...
  useEffect(() => {
    ...
    if (snapshot === lastSavedSnapshotRef.current || !formData.nome.trim()) return;
        ...
        <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
          ...
          <div className="space-y-1">
            ...
            <div className="flex gap-1.5">
              <Input value={formData.cidade} onChange={(e) => updateField("cidade", e.target.value)} placeholder="Cidade" className="h-9 flex-1 text-sm" />
              <Input value={formData.estado} onChange={(e) => updateField("estado", e.target.value.toUpperCase().slice(0, 2))} placeholder="UF" className="h-9 w-14 text-sm" />
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-3">
        ...
        <div className="space-y-3">
          {formData.birthday_children.map((child, idx) => (
            <div key={idx} className="space-y-2 rounded-lg border border-border/40 bg-muted/20 p-3">
              <div className="flex items-center justify-between">
                <span className="text-[11px] font-semibold uppercase tracking-wider text-muted-foreground">
                  Aniversariante {formData.birthday_children.length > 1 ? idx + 1 : ""}
                </span>
                {formData.birthday_children.length > 1 && (
                  <Button type="button" variant="ghost" size="icon" className="h-6 w-6 p-0 text-destructive hover:bg-destructive/10 hover:text-destructive" onClick={() => removeChild(idx)}>
              ...
              <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
                ...
                <div className="space-y-1">
                  ...
                  <div className="relative">
                    <Input value={child.age} onChange={(e) => updateChild(idx, "age", e.target.value.replace(/\D/g, ""))} placeholder="Ex: 5" className="h-9 pr-14 text-sm" />
                    <span className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 text-sm text-muted-foreground">anos</span>
                  </div>
                </div>
                <div className="space-y-1">
          ...
          <Button type="button" variant="outline" size="sm" className="w-full gap-1.5 text-xs" onClick={addChild}>
        ...
        <div className="mt-3 space-y-3 border-t border-border/40 pt-3">
          ...
          {formData.responsaveis.map((responsavel, idx) => (
            <div key={idx} className="space-y-2 rounded-lg border border-border/40 bg-muted/20 p-3">
              <p className="text-[11px] font-semibold uppercase tracking-wider text-muted-foreground">{idx + 1}º Responsável</p>
              <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
                <div className="space-y-1">
                  <Label className="text-xs text-muted-foreground">Nome</Label>
                  <Input value={responsavel.name} onChange={(e) => updateResponsible(idx, "name", e.target.value)} placeholder="Nome" className="h-9 text-sm" />
                </div>
                <div className="space-y-1">
                  <Label className="text-xs text-muted-foreground">Telefone</Label>
                  <Input value={responsavel.phone} onChange={(e) => updateResponsible(idx, "phone", e.target.value)} placeholder="Telefone" className="h-9 text-sm" />
                </div>
                <div className="space-y-1">
                  <Label className="text-xs text-muted-foreground">Parentesco</Label>
                  <Select value={responsavel.relation || "none"} onValueChange={(value) => updateResponsible(idx, "relation", value === "none" ? "" : value)}>
        ...
        <div className="flex items-center gap-2 text-xs text-muted-foreground">
          ...
          {saved && !saving && (
            <>
              ...
              <span className="text-foreground">Salvo automaticamente</span>
            </>
          )}
          {!saving && !saved && <span>Salva após alguns segundos sem digitar</span>}
        </div>
      </div>
    </div>
  );
