                ...
                <Card className="bg-card border-border">
                  <CardContent className="p-4 space-y-3">
                    ...
                    <div className="flex items-center gap-2 flex-wrap border-t border-border/50 pt-3">
                      ...
                      <Button variant="outline" size="sm" className="h-8 gap-1.5 text-xs rounded-full px-3.5 bg-muted/30 border-border/60 hover:bg-primary/10 hover:text-primary hover:border-primary/30 transition-all" onClick={() => setEvalDialogRecord(record)}>
                        <Star className="h-3.5 w-3.5 text-yellow-500" /> Avaliar
                      </Button>
                      <Button variant="outline" size="sm" className="h-8 gap-1.5 text-xs rounded-full px-3.5 bg-muted/30 border-border/60 hover:bg-primary/10 hover:text-primary hover:border-primary/30 transition-all" onClick={() => { const baseUrl = currentCompany?.custom_domain ? `https://${currentCompany.custom_domain}` : window.location.origin; window.open(`${baseUrl}/equipe/${record.id}`, '_blank'); }}>
                        <ExternalLink className="h-3.5 w-3.5" /> Abrir
                      </Button>
                      <Button variant="outline" size="sm" className="h-8 gap-1.5 text-xs rounded-full px-3.5 bg-muted/30 border-border/60 hover:bg-primary/10 hover:text-primary hover:border-primary/30 transition-all" onClick={() => { const domain = currentCompany?.custom_domain || ''; const link = domain ? `${domain}/equipe/${record.id}` : `${window.location.origin}/equipe/${record.id}`; navigator.clipboard.writeText(link); toast({ title: "Link copiado!" }); }}>
