...
DECLARE
  ...
  v_dups_removed int; v_total_dups_removed int := 0;
BEGIN
  FOR v_pair IN
    WITH normalized AS (
  ...
  LOOP
    ...
    FOR i IN 2..array_length(v_pair.ids, 1) LOOP
      ...
      -- 🛡️ STEP 1: Delete from loser any message_id that already exists in winner
      -- (prevents unique constraint violation on (conversation_id, message_id))
      DELETE FROM wapi_messages lm
      WHERE lm.conversation_id = v_loser_id
        AND EXISTS (
          SELECT 1 FROM wapi_messages wm
          WHERE wm.conversation_id = v_winner_id
            AND wm.message_id = lm.message_id
        );
      GET DIAGNOSTICS v_dups_removed = ROW_COUNT;
      v_total_dups_removed := v_total_dups_removed + v_dups_removed;

      -- STEP 2: Move remaining messages from loser to winner
  ...
  RETURN jsonb_build_object(
    'pairs_merged', v_total_pairs,
    'messages_moved', v_total_messages_moved,
    'duplicate_messages_removed', v_total_dups_removed
  );
END $function$;
...
DECLARE
  v_result jsonb;
BEGIN
  SELECT public.merge_duplicate_conversations_intra_instance() INTO v_result;
  RAISE NOTICE 'Merge result: %', v_result;
END $$;