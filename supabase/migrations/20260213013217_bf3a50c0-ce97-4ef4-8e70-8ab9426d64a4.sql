...
BEGIN
  ...
  -- Penalidade por respostas invalidas do bot (-5 por resposta)
  SELECT count(*) INTO v_invalid_replies
  FROM public.lead_history
  WHERE lead_id = _lead_id AND action = 'bot_invalid_reply';

  IF v_invalid_replies > 0 THEN
    v_score := v_score - (v_invalid_replies * 5);
  END IF;
