CREATE INDEX IF NOT EXISTS idx_wapi_messages_quoted_message_id
  ON public.wapi_messages(quoted_message_id)
  WHERE quoted_message_id IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_wapi_conversations_pinned_message_id
  ON public.wapi_conversations(pinned_message_id)
...
DECLARE
  ...
  v_cleared_quotes integer := 0;
  v_cleared_pins integer := 0;
...
BEGIN
  ...
  LOOP
    WITH target_messages AS (
      ...
      UPDATE public.wapi_messages wm
      SET quoted_message_id = NULL
      FROM target_messages tm
      WHERE wm.quoted_message_id = tm.id
      RETURNING wm.id
    )
    SELECT count(*)::integer INTO v_rows FROM updated;

    v_cleared_quotes := v_cleared_quotes + v_rows;
    EXIT WHEN v_rows = 0;
  END LOOP;
  ...
  LOOP
    WITH target_messages AS (
      ...
      FROM target_messages tm
      WHERE wc.pinned_message_id = tm.id
      RETURNING wc.id
    )
    SELECT count(*)::integer INTO v_rows FROM updated;

    v_cleared_pins := v_cleared_pins + v_rows;
    EXIT WHEN v_rows = 0;
  END LOOP;
  ...
  LOOP
    WITH batch AS (
      ...
      JOIN public.wapi_conversations wc ON wc.id = wm.conversation_id
      WHERE wc.instance_id = _instance_id
      ORDER BY wm.created_at NULLS FIRST, wm.id
      LIMIT v_batch_size
    )
  ...
  RETURN jsonb_build_object(
    ...
    'cleared_quotes', v_cleared_quotes,
    'cleared_pins', v_cleared_pins
  );
END;
$$;
