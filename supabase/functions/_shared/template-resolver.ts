...
const VARIABLE_CATALOG: Record<string, { resolver: VariableResolver }> = {
  ...
  convidados: { resolver: (ctx) => ctx.event?.guest_count?.toString() || ctx.lead?.guests || '' },
  ...
  quantidade_pessoas: { resolver: (ctx) => ctx.contract?.quantidade_pessoas || ctx.event?.guest_count?.toString() || ctx.lead?.guests || '' },
